...
import operator
from schemas import (
UserIntent, SessionState,AnswerResponse, SummarizationResponse,
 CalculationResponse, UpdateMemoryResponse)
from prompts import get_intent_classification_prompt,\
      get_chat_prompt_template, MEMORY_SUMMARY_PROMPT
...
def invoke_react_agent(response_schema: type[BaseModel], 
                       messages: List[BaseMessage], llm,
                         tools) -> tuple[Dict[str, Any], List[str]]:
    """
    Wrapper for result and tools used.
    """
...
def classify_intent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    prompt_with_format = prompt.format(user_input=state["user_input"],
                                       conversation_history=history)
    llm_with_structure = llm.with_structured_output(UserIntent)
    response = llm_with_structure.invoke(prompt_with_format)
...
def qa_agent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    result, tools_used = invoke_react_agent(AnswerResponse, messages,
                                             llm, tools)
...
def summarization_agent(state: AgentState,
    ...
    result, tools_used = invoke_react_agent(SummarizationResponse,
                                             messages, llm, tools)
    return {
        "messages": result.get("messages", []),
...
def calculation_agent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    result, tools_used = invoke_react_agent(CalculationResponse, messages,
                                             llm, tools)
...
def create_workflow(llm, tools, debug:bool=False):
    """
    ...
    return workflow.compile(checkpointer=InMemorySaver(),debug=graph_debug)